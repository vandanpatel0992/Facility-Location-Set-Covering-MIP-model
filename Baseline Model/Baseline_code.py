# -*- coding: utf-8 -*-
"""
Created on Sun Nov 19 23:56:20 2017

@author: Vandan
"""
#BASELINE MODEL: minimize production and transportation cost from warehouse to customers
...
readDist = csv.reader(open('DistPlantCustomer.csv'))
...
for i in range(1,5):
		for k in range(1,6):
			m.addConstr(quicksum((x[(i,j,k)] + y[(i,j,k)]) for j in range(1,51)) <= p[(i,k)]*cap[(i,k)])
...
#product1: k=1
#regular
m.addConstr(quicksum(x[(1,j,1)] for j in range(1,51)) <= reghours*100)
#over time
m.addConstr(quicksum(y[(1,j,1)] for j in range(1,51)) <= ovrhours*100)
...
#product2: k=2
#regular
m.addConstr(quicksum(x[(2,j,2)] for j in range(1,51)) <= reghours*50)
#over time
m.addConstr(quicksum(y[(2,j,2)] for j in range(1,51)) <= ovrhours*50)

#plant3: i=3
#production rate: 50 tonnes/hour
#product3: k=3
#regular
m.addConstr(quicksum(x[(3,j,3)] for j in range(1,51)) <= reghours*50)
#over time
m.addConstr(quicksum(y[(3,j,3)] for j in range(1,51)) <= ovrhours*50)
						
#plant4: i=4
#production rate: 50 tonnes/hour
#product4: k=4
#regular
m.addConstr(quicksum(x[(4,j,4)] for j in range(1,51)) <= reghours*50)
#over time
m.addConstr(quicksum(y[(4,j,4)] for j in range(1,51)) <= ovrhours*50)			

#plant4: i=4
#production rate: 50 tonnes/hour
#product5: k=5
#regular
m.addConstr(quicksum(x[(4,j,5)] for j in range(1,51)) <= reghours*50)
#over time
m.addConstr(quicksum(y[(4,j,5)] for j in range(1,51)) <= ovrhours*50)				
...
m.setObjective(quicksum((reg[k]*x[(i,j,k)]+ovr[k]*y[(i,j,k)]) for i in range(1,5) for j in range(1,51) for k in range(1,6)) + quicksum(((x[(i,j,k)]+y[(i,j,k)])*t*dist[(i,j)]) for i in range(1,5) for j in range(1,51) for k in range(1,6)))
...
#import pandas as pd
#pd.DataFrame(x_flow, index=[0]).to_csv('x_bflow.csv')
#pd.DataFrame(y_flow, index=[0]).to_csv('y_bflow.csv')